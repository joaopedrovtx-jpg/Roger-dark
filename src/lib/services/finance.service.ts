 ...
 * Fluxo saque: débito atômico DB → adquirente → row MySQL (fonte da verdade).
 */
...
/** Persiste saque no MySQL (idempotente por id/providerId). */
async function persistWithdrawalDb(
  ...
  try {
    ...
    log.error("withdrawal_persist_failed", {
      id,
...
export async function createWithdrawal(
  ...
  // ALLOW_MOCK_DATA=1: saque interno só em dev
  if (isDatabaseConfigured()) {
...
export function setWithdrawalStatus(
  ...
  if (tx) {
    tx.status =
      status === "pago" ? "pago" : status === "recusado" ? "recusado" : tx.status;
  }
...
/** Snapshot mock/local — rotas preferem getSellerFinance (DB). */
...
/** Preferência DB → mock (somente se ALLOW_MOCK_DATA=1). */
export async function getFinanceSnapshotPreferDb(sellerId: string) {
  const { getSellerFinance } = await import("@/lib/server/db/seller.service");
